...
typedef NS_ENUM(NSUInteger, AgoraRtcRenderMode) {
  /**
   Hidden(1): If the video size is different than that of the display window,
   crops the borders of the video (if the video is bigger) or stretch the video
   (if the video is smaller) to fit it in the window.
   */
  AgoraRtc_Render_Hidden = 1,

  /**
   AgoraRtc_Render_Fit(2): If the video size is different than that of the
   display window, resizes the video proportionally to fit the window.
   */
  AgoraRtc_Render_Fit = 2,

  /**
   AgoraRtc_Render_Adaptive(3)：If both callers use the same screen orientation,
   i.e., both use vertical screens or both use horizontal screens, the
   AgoraRtc_Render_Hidden mode applies; if they use different screen
   orientations, i.e., one vertical and one horizontal, the AgoraRtc_Render_Fit
   mode applies.
   */
...
__attribute__((visibility("default"))) @interface AgoraRtcVideoCanvas
    : NSObject

      /**
       *  The video display view. The SDK does not maintain the lifecycle of the
       view.
          The view can be safely released after calling leaveChannel with a
       returned value.
          The SDK keeps a cache of the view value, so 'setupLocalVideo' to set
       the view value to NULL could be able to clear cache before switching or
       releasing view.
       */
      @property(strong, nonatomic) VIEW_CLASS *view;
@property(assign, nonatomic) AgoraRtcRenderMode
    renderMode; // the render mode of view: hidden, fit and adaptive
@property(assign, nonatomic) NSUInteger uid; // the user id of view
@end

    __attribute__((visibility("default"))) @interface AgoraRtcStats
    : NSObject @property(assign, nonatomic) NSUInteger duration;
...
@end
    ...
    __attribute__((visibility("default"))) @interface AgoraRtcLocalVideoStats
    : NSObject @property(assign, nonatomic) NSUInteger sentBitrate;
@property(assign, nonatomic) NSUInteger sentFrameRate;
@end

    __attribute__((visibility("default"))) @interface AgoraRtcRemoteVideoStats
...
@end
    ...
    __attribute__((visibility("default"))) @interface AgoraRtcAudioVolumeInfo
    : NSObject @property(assign, nonatomic) NSUInteger uid;
...
/**
 *  The warning occurred in SDK. The APP could igonre the warning, and the SDK
 * could try to resume automically.
 *
 *  @param engine      The engine kit
 *  @param warningCode The warning code
 */
- (void)rtcEngine:(AgoraRtcEngineKit *)engine
  didOccurWarning:(AgoraRtcWarningCode)warningCode;

/**
 *  The error occurred in SDK. The SDK couldn't resume to normal state, and the
 * app need to handle it.
 *
 *  @param engine    The engine kit
...
/**
 *  The sdk reports the volume of a speaker. The interface is disable by
 * default, and it could be enable by API "enableAudioVolumeIndication"
 *
 *  @param engine      The engine kit
 ...
 */
- (void)rtcEngine:(AgoraRtcEngineKit *)engine
    localVideoStatWithSentBitrate:(NSInteger)sentBitrate
                    sentFrameRate:(NSInteger)sentFrameRate __deprecated;
- (void)rtcEngine:(AgoraRtcEngineKit *)engine
 ...
 */
- (void)rtcEngine:(AgoraRtcEngineKit *)engine
    remoteVideoStatOfUid:(NSUInteger)uid
                   delay:(NSInteger)delay
         receivedBitrate:(NSInteger)receivedBitrate
       receivedFrameRate:(NSInteger)receivedFrameRate __deprecated;
- (void)rtcEngine:(AgoraRtcEngineKit *)engine
...
/**
 *  Event of disconnected with server. This event is reported at the moment SDK
 * loses connection with server.
 *  In the mean time SDK automatically tries to reconnect with the server until
 * APP calls leaveChannel.
 *
 *  @param engine The engine kit
 */
...
/**
 *  Event of loss connection with server. This event is reported after the
 * connection is interrupted and exceed the retry period (10 seconds by
 * default).
 *  In the mean time SDK automatically tries to reconnect with the server until
 * APP calls leaveChannel.
 *
 ...
 *
 *  @param engine The engine kit
 *  @param stats  The statistics of rtc status, including duration, sent bytes
 * and received bytes
 */
- (void)rtcEngine:(AgoraRtcEngineKit *)engine
 ...
 *
 *  @param engine The engine kit
 *  @param stats  The statistics of the call, including duration, sent bytes and
 * received bytes
 */
- (void)rtcEngine:(AgoraRtcEngineKit *)engine
    didLeaveChannelWithStats:(AgoraRtcStats *)stats;
...
audioQualityOfUid:(NSUInteger)uid
          quality:(AgoraRtcQuality)quality
 ...
 *
 *  @param key      The vendor key is issued to the application developers by
 * Agora.
 *  @param delegate
 *
 ...
 */
+ (instancetype)
sharedEngineWithVendorKey:(NSString *)key
...
- (void)audioVolumeIndicationBlock:
    (void (^)(NSArray *speakers,
              NSInteger totalVolume))audioVolumeIndicationBlock __deprecated;

- (void)firstLocalVideoFrameBlock:
    (void (^)(NSInteger width, NSInteger height,
              NSInteger elapsed))firstLocalVideoFrameBlock __deprecated;
- (void)firstRemoteVideoDecodedBlock:
    (void (^)(NSUInteger uid, NSInteger width, NSInteger height,
              NSInteger elapsed))firstRemoteVideoDecodedBlock __deprecated;
- (void)firstRemoteVideoFrameBlock:
    (void (^)(NSUInteger uid, NSInteger width, NSInteger height,
              NSInteger elapsed))firstRemoteVideoFrameBlock __deprecated;
- (void)userJoinedBlock:
    (void (^)(NSUInteger uid, NSInteger elapsed))userJoinedBlock __deprecated;
- (void)userOfflineBlock:(void (^)(NSUInteger uid))userOfflineBlock
    __deprecated;
- (void)userMuteAudioBlock:
    (void (^)(NSUInteger uid, BOOL muted))userMuteAudioBlock __deprecated;
- (void)userMuteVideoBlock:
    (void (^)(NSUInteger uid, BOOL muted))userMuteVideoBlock __deprecated;
- (void)localVideoStatBlock:
    (void (^)(NSInteger sentBitrate,
              NSInteger sentFrameRate))localVideoStatBlock __deprecated;
- (void)remoteVideoStatBlock:
    (void (^)(NSUInteger uid, NSInteger delay, NSInteger receivedBitrate,
              NSInteger receivedFrameRate))remoteVideoStatBlock __deprecated;
- (void)cameraReadyBlock:(void (^)())cameraReadyBlock __deprecated;
- (void)connectionLostBlock:(void (^)())connectionLostBlock __deprecated;
- (void)rejoinChannelSuccessBlock:
    (void (^)(NSString *channel, NSUInteger uid,
              NSInteger elapsed))rejoinChannelSuccessBlock __deprecated;
...
- (void)leaveChannelBlock:(void (^)(AgoraRtcStats *stat))leaveChannelBlock
    __deprecated;
- (void)audioQualityBlock:
    (void (^)(NSUInteger uid, AgoraRtcQuality quality, NSUInteger delay,
              NSUInteger lost))audioQualityBlock __deprecated;
- (void)networkQualityBlock:
    (void (^)(AgoraRtcQuality quality))networkQualityBlock __deprecated;
...
/**
 *  Create an open UDP socket to the AgoraRtcEngineKit cloud service to join a
 channel.
 ...
 *
 *  @param key               Account level credentials issued by AgoraLab
 company to each vendor.
 *  @param channelName       Joining in the same channel indicates those clients
 have entered in one room.
 *  @param info              Optional, this argument can be whatever the
 programmer likes personally.
 *  @param uid               Optional, this argument is the unique ID for each
 member in one channel.
                             If not specified, or set to 0, the SDK
 automatically allocates an ID, and the id could be gotten in
 ...
 *  lets the user leave a channel, i.e., hanging up or exiting a call.
    After joining a channel, the user must call the leaveChannel method to end
 the call before joining another one.
    It is synchronous, i.e., it only returns until the call ends and all
 resources are released.
 *  @param leaveChannelBlock indicate the statistics of this call, from
 joinChannel to leaveChannel, including duration, tx bytes and rx bytes in the
 call.
 *
...
/**
 *  Enables the network quality test. When enabled, the callback
 'networkQualityBlock' notifies the application about the user's network
 connection quality.
 Note: Once the network test is enabled, it uses the network bandwidth even when
 the application is not in a call.
 Recommandation: When the application is foreground, enable the network
 connection test; and when the application is switched to background, disable
 network test  in order to reduce network traffic.
 ...
 *
 *  @return 0 when this method is called successfully, or minus value when this
 method failed.
 */
- (int)enableVideo;
...
/**
 *  The call id of current client. The call id could be used in 'rate' and
 * 'complain' method.
 *
 ...
 *  @param rating      The rating for the call between 1 (lowest score) to 10
 * (highest score).
 *  @param description Optional, decribed by user for the call with a length
 * less than 800 bytes.
 *
 *  @return 0 when executed successfully. return ERR_INVALID_ARGUMENT (-2)：The
 * passed argument is invalid, e.g., callId invalid. return ERR_NOT_READY
 * (-3)：The SDK status is incorrect, e.g., initialization failed.
 */
 ...
 *
 *  @return YES when speakerphone is enabled. NO when speakerphone is not
 * enabled.
 */
- (BOOL)isSpeakerphoneEnabled;

/**
 *  Sets the speakerphone volume. The speaker volume could be adjust by
 * MPMusicPlayerController and other iOS API easily.
 *
 ...
 *
 *  @param interval Specifies the time interval between two consecutive volume
 indications.
                    <=0: Disables volume indication.
                    >0 : The volume indication interval in milliseconds.
 Recommandation: >=200ms.
...
/**
 *  Launches an echo test to test if the audio devices (e.g., headset and
 speaker) and the network connection work properly. In the test, the user speaks
 first, and the recording will be played back in 10 seconds. If the user can
 hear what he said in 10 seconds, it indicates that the audio devices and
 network connection work properly.
 Notes: Must call stopEchoTest to end the test, or won't start new test or join
 channel.
 *
 *  @param successBlock
 *
 *  @return 0 when executed successfully. return minus value if failed. e.g.
 ERR_REFUSED (-5)：Failed to launch the echo test, e.g., initialization failed.
 */
 ...
 *
 *  @return 0 when executed successfully. return minus value if failed. e.g.
 * ERR_REFUSED(-5)：Failed to stop the echo test. It could be that the echo test
 * is not running.
 */
- (int)stopEchoTest;
 ...
 *
 *  @param mute true: Mutes all remote received audio. false: Unmutes all remote
 * received audio.
 *
 *  @return 0 when executed successfully. return minus value if failed.
 */
...
/**
 *  Set up the local video view. The video canvus is initialized with video
 * display setting. And it could be called before entering a channel.
 *
 *  @param local the canvas is composed of view, renderMode and uid. How to
 * initialize 'local'? please take a look at 'AgoraRtcVideoCanvas'
 *
 *  @return 0 when executed successfully. return minus value if failed.
 */
...
/**
 *  Set up the remote video view. The video canvus is initialized with video
 * display setting. It could be called after receiving the remote video streams
 * to configure the video settings.
 *
 *  @param remote the canvas is composed of view, renderMode and uid. How to
 * initialize 'remote'? please take a look at 'AgoraRtcVideoCanvas'
 *
 *  @return 0 when executed successfully. return minus value if failed.
 */
...
/**
 *  Configure display setting of local view. And it could be called mutiple
 * times during a call.
 *
 *  @param mode There are Hidden(1), Fit(2) and Adaptive(3) mode. Please take a
 * look at definition of enum AgoraRtcRenderMode
 *
 *  @return 0 when executed successfully. return minus value if failed.
 */
...
/**
 *  Configure display setting of remote view. And it could be called mutiple
 * times during a call.
 *
...
/**
 *  API deprecated. Please refer to open source
 * https://github.com/AgoraLab/AgoraDemo, the function "onSwitchRemoteUsers" in
 * demo agora-easycall-ios-oc
 ...
 *  @return 0 when executed successfully. return minus value if failed.
 */
- (int)switchView:(NSUInteger)uid1 andAnother:(NSUInteger)uid2 __deprecated;
 ...
 *
 *  @param profile enumeration definition about the video resolution, fps and
 * max kbps
 *
 *  @return 0 when executed successfully. return minus value if failed.
 */
 ...
 *
 *  @param mute YES: stop sending local video stream to the network, NO: start
 * sending local video stream.
 *
 *  @return 0 when executed successfully. return minus value if failed.
 */
 ...
 *
 *  @param uid  The remote user id
 *  @param mute YES: discard the video stream from remote user, NO: start
...
/**
 *  Renew dynamic key, refresh the new key into agora engine. APP should call
 * this API when SDK reports error ERR_DYNAMIC_KEY_TIMEOUT.
 *
...
/**
 *  Set the role of user: such as broadcaster, audience and free(as broadcaster
