...
- (void)viewDidLoad {
  ...
  [self.messageArray enumerateObjectsUsingBlock:^(id obj, NSUInteger idx,
                                                  BOOL *stop) {
    ...
    } else if (![((RCContactNotificationMessage *)message.content)
                       .operation
                   isEqualToString:
                       ContactNotificationMessage_ContactOperationRequest]) {
      [weakSelf.messageArray removeObject:obj];
    }
  }];
...
- (void)tableView:(UITableView *)tableView
  ...
  if (editingStyle == UITableViewCellEditingStyleDelete) {
    ...
    if ([[RCIMClient sharedRCIMClient]
            deleteMessages:@[ [NSNumber numberWithLong:message.messageId] ]]) {
...
- (UITableViewCell *)tableView:(UITableView *)tableView
  ...
  RCDFriendInvitationTableViewCell *cell =
      [tableView dequeueReusableCellWithIdentifier:
                     @"rCDFriendInvitationTableViewCellIdentifier"
