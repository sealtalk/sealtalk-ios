...
+ (SInt32)FMDBVersion {
  ...
  // we go through these hoops so that we only have to change the version number
  // in a single spot.
  ...
  dispatch_once(&once, ^{
    ...
    NSString *junk =
        [prodVersion stringByReplacingOccurrencesOfString:@"." withString:@""];
...
static int FMDBDatabaseBusyHandler(void *f, int count) {
  ...
  NSTimeInterval delta =
      [NSDate timeIntervalSinceReferenceDate] - (self->_startBusyRetryTime);

  if (delta < [self maxBusyRetryTimeInterval]) {
    int requestedSleepInMillseconds = (int)arc4random_uniform(50) + 50;
    int actualSleepInMilliseconds = sqlite3_sleep(requestedSleepInMillseconds);
    if (actualSleepInMilliseconds != requestedSleepInMillseconds) {
      NSLog(@"WARNING: Requested sleep of %i milliseconds, but SQLite returned "
            @"%i. Maybe SQLite wasn't built with HAVE_USLEEP=1?",
...
- (void)setMaxBusyRetryTimeInterval:(NSTimeInterval)timeout {
  ...
  if (timeout > 0) {
    sqlite3_busy_handler(_db, &FMDBDatabaseBusyHandler,
                         (__bridge void *)(self));
  } else {
...
- (FMResultSet *)executeQuery:(NSString *)sql
  ...
  if (!pStmt) {
    ...
    if (SQLITE_OK != rc) {
      if (_logsErrors) {
        NSLog(@"DB Error: %d \"%@\"", [self lastErrorCode],
  ...
  if (dictionaryArgs) {
    ...
    for (NSString *dictionaryKey in [dictionaryArgs allKeys]) {
      ...
      if (_traceExecution) {
        NSLog(@"%@ = %@", parameterName,
              [dictionaryArgs objectForKey:dictionaryKey]);
      }
...
int FMDBExecuteBulkSQLCallback(void *theBlockAsVoid, int columns, char **values,
...
int FMDBExecuteBulkSQLCallback(void *theBlockAsVoid, int columns, char **values,
  ...
  for (NSInteger i = 0; i < columns; i++) {
    ...
    id value =
        values[i] ? [NSString stringWithUTF8String:values[i]] : [NSNull null];
...
- (BOOL)executeStatements:(NSString *)sql
  ...
  rc = sqlite3_exec([self sqliteHandle], [sql UTF8String],
                    block ? FMDBExecuteBulkSQLCallback : nil,
...
- (BOOL)startSavePointWithName:(NSString *)name error:(NSError **)outErr {
  ...
  NSString *sql = [NSString
      stringWithFormat:@"savepoint '%@';", FMDBEscapeSavePointName(name)];
...
void FMDBBlockSQLiteCallBackFunction(
