...
{
    [RCDHTTPTOOL getFriends:userId complete:^(NSMutableArray *result) {
        completion(result);
    }];
}
...
//                RCUserInfo *friend = [[RCUserInfo alloc] initWithUserId:userInfo.userId name:userInfo.name portrait:userInfo.portraitUri];
                RCDUserInfo *friend = [[RCDUserInfo alloc] initWithUserId:userInfo.userId name:userInfo.name portrait:userInfo.portraitUri];
