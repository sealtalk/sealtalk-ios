...
- (void)setVersionUrl:(NSString *)versionUrl {
  ...
  [[NSUserDefaults standardUserDefaults] setObject:self.versionUrl
                                            forKey:@"newVersionUrl"];
  [[NSUserDefaults standardUserDefaults] synchronize];
}
...
- (void)connectionDidFinishLoading:(NSURLConnection *)connection {
  NSDate *now = [[NSDate alloc] init];
  [[NSUserDefaults standardUserDefaults] setInteger:now.timeIntervalSince1970
...
- (void)compareVersion:(NSString *)receiveStr {
  ...
  NSRange startRange = [receiveStr rangeOfString:@"<a href='itms-services:"
                                         options:0
                                           range:strRange];
  if (startRange.location != NSNotFound) {
    NSRange endRange = [receiveStr
        rangeOfString:@"'>"
              options:0
                range:NSMakeRange(startRange.location + startRange.length,
                                  receiveStr.length - startRange.location -
                                      startRange.length)];
    NSString *url = [receiveStr
        substringWithRange:NSMakeRange(startRange.location + 9,
                                       endRange.location - startRange.location -
                                           9)];
    NSRange nameEndRange = [receiveStr
        rangeOfString:@"</a>"
              options:0
                range:NSMakeRange(endRange.location,
                                  receiveStr.length - endRange.location)];
    NSString *name =
        [receiveStr substringWithRange:NSMakeRange(endRange.location + 2,
    ...
    NSString *versionString =
        [name substringWithRange:NSMakeRange(versionBegin.location + 1, 12)];
...
- (void)compareVersion:(NSString *)receiveStr {
  ...
  NSCharacterSet *setToRemove = [[NSCharacterSet
  ...
  while (true) {
    NSRange startRange =
        [receiveStr rangeOfString:@"itms-services:" options:0 range:strRange];
    if (startRange.location != NSNotFound) {
      NSRange endRange = [receiveStr
          rangeOfString:@"')\""
                options:0
                  range:NSMakeRange(startRange.location + startRange.length,
                                    receiveStr.length - startRange.location -
                                        startRange.length)];
      NSString *url =
          [receiveStr substringWithRange:NSMakeRange(startRange.location,
                                                     endRange.location -
                                                         startRange.location)];

      NSRange nameStartRange = [receiveStr
          rangeOfString:@">"
                options:0
                  range:NSMakeRange(endRange.location,
                                    receiveStr.length - endRange.location)];
      ...
      NSRange nameEndRange = [receiveStr
          rangeOfString:@"</a>"
                options:0
                  range:NSMakeRange(endRange.location,
                                    receiveStr.length - endRange.location)];
      NSString *name = [receiveStr
          substringWithRange:NSMakeRange(nameStartRange.location + 1,
                                         nameEndRange.location -
                                             nameStartRange.location - 1)];
...
- (void)updateNewVersionBadge {
  ...
  if (self.hasNewVersion) {
    ...
    _versionLb.attributedText = [[NSAttributedString alloc]
