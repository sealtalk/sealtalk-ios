...
- (void)viewDidLoad {
  ...
  [_ivAva sd_setImageWithURL:[NSURL URLWithString:_userInfo.portraitUri]
  ...
  [RCDHTTPTOOL isMyFriendWithUserInfo:_userInfo
...
- (IBAction)actionAgree:(id)sender {
  ...
  [AFHttpTool processRequestFriend:weakSelf.userInfo.userId
      withIsAccess:YES
      success:^(id response) {
...
- (IBAction)actionDisagree:(id)sender {
  ...
  ;
  [alertView show];
  //    [AFHttpTool processRequestFriend:self.userInfo.userId withIsAccess:NO
  //    success:^(id response) {
  //
  //    } failure:^(NSError *err) {
  //
