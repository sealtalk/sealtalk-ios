...
- (IBAction)onAgree:(id)sender {
  ...
  RCContactNotificationMessage *_contactNotificationMsg =
      (RCContactNotificationMessage *)weakSelf.model.content;
  ...
  hud.labelText = @"添加中...";
  [AFHttpTool processRequestFriend:_contactNotificationMsg.sourceUserId
      withIsAccess:YES
      success:^(id response) {
...
- (void)setModel:(RCMessage *)model {
  ...
  RCContactNotificationMessage *_contactNotificationMsg =
      (RCContactNotificationMessage *)model.content;
  ...
  if (_cache_userinfo) {
    ...
    [self.portrait
        sd_setImageWithURL:[NSURL URLWithString:_cache_userinfo[@"portraitUri"]]
          placeholderImage:[UIImage imageNamed:@"icon_person"]];
  } else {
    self.userName.text = [NSString
        stringWithFormat:@"user<%@>", _contactNotificationMsg.sourceUserId];
    [self.portrait setImage:[UIImage imageNamed:@"icon_person"]];
  }

  __weak RCDFriendInvitationTableViewCell *weakSelf = self;
  RCDUserInfo *_userInfo =
      [[RCDUserInfo alloc] initWithUserId:_contactNotificationMsg.sourceUserId
                                     name:@""
                                 portrait:@""];
  [RCDHTTPTOOL
      isMyFriendWithUserInfo:_userInfo
                  completion:^(BOOL isFriend) {
                    dispatch_async(dispatch_get_main_queue(), ^{
