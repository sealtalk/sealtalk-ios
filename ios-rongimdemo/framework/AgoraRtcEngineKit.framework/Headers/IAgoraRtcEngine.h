...
template <class T> class AutoPtr {
  ...
  public:
    ...
    template <class C1, class C2> bool queryInterface(C1 *c, C2 iid) {
        pointer_type p = NULL;
...
struct VideoCompositingLayout {
    struct Region {
        ...
        Region() : uid(0), x(0), y(0), width(0), height(0), zOrder(0), alpha(1.0), renderMode(RENDER_MODE_HIDDEN) {}
    };
    int canvasWidth;
    int canvasHeight;
    ...
    VideoCompositingLayout()
        : canvasWidth(0), canvasHeight(0), backgroundColor(NULL), regions(NULL), regionCount(0), appData(NULL),
          appDataLength(0) {}
};
...
struct PublisherConfiguration {
    ...
    PublisherConfiguration()
        : width(640), height(360), framerate(15), bitrate(500), defaultLayout(1),
          lifecycle(RTMP_STREAM_LIFE_CYCLE_BIND2CHANNEL), owner(true), injectStreamWidth(0), injectStreamHeight(0),
          injectStreamUrl(NULL), publishUrl(NULL), rawStreamUrl(NULL), extraInfo(NULL) {}
};
#endif
...
class IRtcEngineEventHandler {
  public:
     ...
     * NOTE: to be compatible with previous version, ERR_CHANNEL_KEY_EXPIRED and ERR_INVALID_CHANNEL_KEY are also
     * reported via onError() callback. You should move renew of channel key logic into this callback.
     */
...
struct RtcEngineContext {
    ...
    const char *appId;
    RtcEngineContext() : eventHandler(NULL), appId(NULL) {}
};
...
class IRtcEngine {
  public:
     ...
     *              APP should try not to call release(true) in the engine's callbacks, call it this way in a separate
     * thread instead. false: notify engine to release its resources and returns without waiting for resources are
     * really destroyed
     */
     ...
     * @param [in] channelKey
     *        the channel key, if you have initialized the engine with an available APP ID, it can be null here. If you
     * enable channel key on the dashboard, specify channel key here
    ...
    /**
     * Specify encryption secret enables built-in encryption function. Leaving channel will clear the secret specified
     * in last channel
     * @param [in] secret
...
class RtcEngineParameters {
  public:
    ...
    int muteLocalAudioStream(bool mute) {
        return setParameters("{\"rtc.audio.mute_me\":%s,\"che.audio.mute_me\":%s}", mute ? "true" : "false",
                             mute ? "true" : "false");
    }
    ...
    int muteLocalVideoStream(bool mute) {
        return setParameters("{\"rtc.video.mute_me\":%s,\"che.video.local.send\":%s}", mute ? "true" : "false",
                             mute ? "false" : "true");
    }

    int enableLocalVideo(bool enabled) {
        return setParameters("{\"rtc.video.capture\":%s,\"che.video.local.capture\":%s,\"che.video.local.render\":%s,"
                             "\"che.video.local.send\":%s}",
    ...
    int startAudioMixing(const char *filePath, bool loopback, bool replace, int cycle) {
        ...
        return setObject("che.audio.start_file_as_playout",
                         "{\"filePath\":\"%s\",\"loopback\":%s,\"replace\":%s,\"cycle\":%d}", filePath,
                         loopback ? "true" : "false", replace ? "true" : "false", cycle);
    }
    /**
    ...
    int getAudioMixingDuration() {
        int duration = 0;
        int r =
            m_parameter ? m_parameter->getInt("che.audio.get_mixing_file_length_ms", duration) : -ERR_NOT_INITIALIZED;
        if (r == 0)
            r = duration;
        return r;
    }
    ...
    int startScreenCapture(unsigned int windowId, int captureFreq, const Rect *rect) {
        if (!rect)
            ...
            return setObject("che.video.start_screen_capture",
                             "{\"id\":%u,\"captureFreq\":%d,\"top\":%d,\"left\":%d,\"bottom\":%d,\"right\":%d}",
                             windowId, captureFreq, rect->top, rect->left, rect->bottom, rect->right);
    }
    ...
    int updateScreenCaptureRegion(const Rect *rect) {
        if (!rect)
            ...
            return setObject("che.video.update_screen_capture_region",
                             "{\"top\":%d,\"left\":%d,\"bottom\":%d,\"right\":%d}", rect->top, rect->left, rect->bottom,
                             rect->right);
    }
#elif defined(_WIN32)
    /**
    ...
    int startScreenCapture(HWND windowId, int captureFreq, const Rect *rect) {
        if (!rect)
            return setObject("che.video.start_screen_capture", "{\"id\":%u,\"captureFreq\":%d}", (unsigned int)windowId,
                             captureFreq);
        else
            return setObject("che.video.start_screen_capture",
                             "{\"id\":%u,\"captureFreq\":%d,\"top\":%d,\"left\":%d,\"bottom\":%d,\"right\":%d}",
                             (unsigned int)windowId, captureFreq, rect->top, rect->left, rect->bottom, rect->right);
    }
    ...
    int startRecordingService(const char *recordingKey) {
        return m_parameter ? m_parameter->setString("rtc.api.start_recording_service", recordingKey)
                           : -ERR_NOT_INITIALIZED;
    }

    int stopRecordingService(const char *recordingKey) {
        return m_parameter ? m_parameter->setString("rtc.api.stop_recording_service", recordingKey)
                           : -ERR_NOT_INITIALIZED;
    }

    int refreshRecordingServiceStatus() {
        return m_parameter ? m_parameter->setBool("rtc.api.query_recording_service_status", true)
                           : -ERR_NOT_INITIALIZED;
    }

    int enableDualStreamMode(bool enabled) {
        return setParameters("{\"rtc.dual_stream_mode\":%s,\"che.video.enableLowBitRateStream\":%s}",
                             enabled ? "true" : "false", enabled ? "true" : "false");
    }

    int setRecordingAudioFrameParameters(int sampleRate, int channel, RAW_AUDIO_FRAME_OP_MODE_TYPE mode,
                                         int samplesPerCall) {
    ...
    int setPlaybackAudioFrameParameters(int sampleRate, int channel, RAW_AUDIO_FRAME_OP_MODE_TYPE mode,
                                        int samplesPerCall) {
    ...
    int setMixedAudioFrameParameters(int sampleRate, int samplesPerCall) {
        return setObject("che.audio.set_mixed_raw_audio_format", "{\"sampleRate\":%d,\"samplesPerCall\":%d}",
    ...
    int setHighQualityAudioParameters(bool fullband, bool stereo, bool fullBitrate) {
        return setObject("che.audio.codec.hq", "{\"fullband\":%s,\"stereo\":%s,\"fullBitrate\":%s}",
                         fullband ? "true" : "false", stereo ? "true" : "false", fullBitrate ? "true" : "false");
    }
    int enableWebSdkInteroperability(bool enabled) { // enable interoperability with zero-plugin web sdk
        return setParameters("{\"rtc.video.web_h264_interop_enable\":%s,\"che.video.web_h264_interop_enable\":%s}",
                             enabled ? "true" : "false", enabled ? "true" : "false");
    }
    // only for live broadcasting
    int setVideoQualityParameters(bool preferFrameRateOverImageQuality) {
        return setParameters("{\"rtc.video.prefer_frame_rate\":%s,\"che.video.prefer_frame_rate\":%s}",
  ...
  protected:
    ...
    int enableLocalVideoSend(bool enabled) { return muteLocalVideoStream(!enabled); }
