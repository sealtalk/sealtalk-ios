...
@property (atomic, YZHMB_STRONG) UIView *indicator;
@property (atomic, YZHMB_STRONG) NSTimer *graceTimer;
@property (atomic, YZHMB_STRONG) NSTimer *minShowTimer;
@property (atomic, YZHMB_STRONG) NSDate *showStarted;
@property (atomic, assign) CGSize size;

@end


@implementation YZHMBProgressHUD {
	BOOL useAnimation;
...
+ (YZHMB_INSTANCETYPE)showHUDAddedTo:(UIView *)view animated:(BOOL)animated {
	YZHMBProgressHUD *hud = [[self alloc] initWithView:view];
	[view addSubview:hud];
	[hud show:animated];
...
+ (BOOL)hideHUDForView:(UIView *)view animated:(BOOL)animated {
	YZHMBProgressHUD *hud = [self HUDForView:view];
	if (hud != nil) {
...
+ (NSUInteger)hideAllHUDsForView:(UIView *)view animated:(BOOL)animated {
	NSArray *huds = [YZHMBProgressHUD allHUDsForView:view];
	for (YZHMBProgressHUD *hud in huds) {
		hud.removeFromSuperViewOnHide = YES;
		[hud hide:animated];
	}
	return [huds count];
...
+ (YZHMB_INSTANCETYPE)HUDForView:(UIView *)view {
	...
	for (UIView *subview in subviewsEnum) {
		if ([subview isKindOfClass:self]) {
			return (YZHMBProgressHUD *)subview;
		}
	}
	return nil;
}
...
- (void)updateIndicators {
	...
	BOOL isRoundIndicator = [indicator isKindOfClass:[YZHRoundProgressView class]];
	...
	if (mode == MBProgressHUDModeIndeterminate &&  !isActivityIndicator) {
		...
		[indicator removeFromSuperview];
        self.indicator = MB_AUTORELEASE([[YZHBarProgressView alloc] init]);
		[self addSubview:indicator];
	}
		...
		if (!isRoundIndicator) {
			...
			[indicator removeFromSuperview];
			self.indicator = MB_AUTORELEASE([[YZHRoundProgressView alloc] init]);
			[self addSubview:indicator];
		}
		if (mode == MBProgressHUDModeAnnularDeterminate) {
			[(YZHRoundProgressView *)indicator setAnnular:YES];
		}
	}
...
@implementation YZHRoundProgressView
...
@implementation YZHBarProgressView
