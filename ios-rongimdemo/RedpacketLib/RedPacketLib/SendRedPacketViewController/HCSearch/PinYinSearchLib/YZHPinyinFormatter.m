...
@interface YZHPinyinFormatter ()
+(NSInteger)getNumericValue:(unichar)c;
+(NSInteger)indexOfChar:(int*) table ch:(unichar)c;
@end    

@implementation YZHPinyinFormatter

static int yzh_numericKeys[] = {
...
static unichar yzh_numericValues[] = {
...
+ (NSString *)yzh_formatHanyuPinyinWithNSString:(NSString *)pinyinStr
  ...
  if ((YZHToneTypeWithToneMark == [outputFormat yzh_toneType]) && ((YZHVCharTypeWithV == [outputFormat yzh_vCharType]) || (YZHVCharTypeWithUAndColon == [outputFormat yzh_vCharType]))) {
  ...
  if (YZHToneTypeWithoutTone == [outputFormat yzh_toneType]) {
  ...
  else if (YZHToneTypeWithToneMark == [outputFormat yzh_toneType]) {
      pinyinStr =[pinyinStr stringByReplacingOccurrencesOfString:@"u:" withString:@"v"];
     pinyinStr = [YZHPinyinFormatter yzh_convertToneNumber2ToneMarkWithNSString:pinyinStr];
  }
  if (YZHVCharTypeWithV == [outputFormat yzh_vCharType]) {
      pinyinStr =[pinyinStr stringByReplacingOccurrencesOfString:@"u:" withString:@"v"];
  }
  else if (YZHVCharTypeWithUUnicode == [outputFormat yzh_vCharType]) {
      pinyinStr =[pinyinStr stringByReplacingOccurrencesOfString:@"u:" withString:@"ü"];
  }
  if (YZHCaseTypeUppercase == [outputFormat yzh_caseType]) {
...
+ (NSString *)yzh_convertToneNumber2ToneMarkWithNSString:(NSString *)pinyinStr {
  ...
  if ([lowerCasePinyinStr yzh_matchesPatternRegexPattern:@"[a-z]*[1-5]?"]) {
    unichar defautlCharValue = '$';
    int defautlIndexValue = -1;
    unichar unmarkedVowel = defautlCharValue;
    long int indexOfUnmarkedVowel = defautlIndexValue;
    unichar charA = 'a';
    unichar charE = 'e';
    NSString *ouStr = @"ou";
    ...
    if ([lowerCasePinyinStr yzh_matchesPatternRegexPattern:@"[a-z]*[1-5]"]) {
      long int tuneNumber = [YZHPinyinFormatter getNumericValue:[lowerCasePinyinStr characterAtIndex:lowerCasePinyinStr.length -1]];
      long int indexOfA = [lowerCasePinyinStr yzh_indexOf:charA];
      long int indexOfE = [lowerCasePinyinStr yzh_indexOf:charE];
      long int ouIndex = [lowerCasePinyinStr yzh_indexOfString:ouStr];
      if (-1 != indexOfA) {
        ...
        for (long int i = [lowerCasePinyinStr length] - 1; i >= 0; i--) {
          if ([[NSString yzh_valueOfChar:[lowerCasePinyinStr characterAtIndex:i]] yzh_matchesPatternRegexPattern:@"[aeiouv]"]) {
      ...
      if ((defautlCharValue != unmarkedVowel) && (defautlIndexValue != indexOfUnmarkedVowel)) {
        long int rowIndex = [allUnmarkedVowelStr yzh_indexOf:unmarkedVowel];
        long int columnIndex = tuneNumber - 1;
        long int vowelLocation = rowIndex * 5 + columnIndex;
...
+(NSInteger)getNumericValue:(unichar)c
{
    ...
    NSInteger result = [self indexOfChar:yzh_numericKeys ch:c];
    if (result >= 0 && c <= yzh_numericValues[result * 2]) {
        unichar difference = yzh_numericValues[result * 2 + 1];
        if (difference == 0) {
            return -2;
        }
