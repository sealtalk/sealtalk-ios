...
//设置会话截屏通知
+ (void)setScreenCaptureNotification:(BOOL)open conversationType:(RCConversationType)type targetId:(NSString *)targetId complete:(void (^)(BOOL))complete {
    [RCDChatAPI setChatConfigWithConversationType:type targetId:targetId screenCaptureNotification:open complete:^(BOOL success) {
        if (success) {
            [RCDChatManager saveScreenCaptureNotification:success conversationType:type targetId:targetId];
        }
        if (complete) {
            complete(success);
        }
...
//获取会话截屏通知状态
+ (void)getScreenCaptureNotification:(RCConversationType)type targetId:(NSString *)targetId complete:(void (^)(BOOL))complete error:(void (^)())error {
    [RCDChatAPI getChatConfigWithConversationType:type targetId:targetId success:^(BOOL open) {
        if (open) {
...
+ (void)setGroupMessageClearStatus:(RCDGroupMessageClearStatus)status
    ...
    [RCDChatAPI setGroupMessageClearStatus:status groupId:groupId complete:^(BOOL success) {
        if (success) {
            [self saveChatConfigWithConversationType:ConversationType_GROUP targetId:groupId groupMessageClearStatus:status];
        }
        if (complete) {
            complete(success);
        }
...
+ (void)getGroupMessageClearStatus:(NSString *)groupId
    ...
    [RCDChatAPI getGroupMessageClearStatus:groupId complete:^(RCDGroupMessageClearStatus status) {
...
+ (BOOL)isHoldChatNotificationMessage:(RCMessage *)message{
    if ([message.content isMemberOfClass:[RCDChatNotificationMessage class]]) {
        ...
        if (![msg.operation isEqualToString:RCDChatNotificationSendScreenNtf]){
            [self getScreenCaptureNotification:message.conversationType targetId:message.targetId complete:^(BOOL open) {
...
#pragma mark - helper
+ (void)saveScreenCaptureNotification:(BOOL)screenCaptureNotification conversationType:(RCConversationType)type targetId:(NSString *)targetId {
    ...
    if (screenCaptureNotification) {
        ...
        if (chatConfig) {
            ...
            chatConfig = [RCDChatConfig chatConfigWithConversationType:type targetId:targetId screenCaptureNotification:screenCaptureNotification];
        }
        [RCDDBManager saveChatConfig:chatConfig];
    }
}

+ (void)saveChatConfigWithConversationType:(RCConversationType)type targetId:(NSString *)targetId groupMessageClearStatus:(RCDGroupMessageClearStatus)status {
    RCDChatConfig *chatConfig = nil;
    chatConfig = [RCDDBManager getChatConfigWithConversationType:type targetId:targetId];
    if (chatConfig) {
        chatConfig.messageClearStatus = status;
    } else {
        chatConfig = [RCDChatConfig chatConfigWithConversationType:type targetId:targetId screenCaptureNotification:NO messageClearStatus:status];
    }
    [RCDDBManager saveChatConfig:chatConfig];
}
@end
