...
- (RCDCSButton *)solvedButton {
    if (!_solvedButton) {
        ...
        _solvedButton = [[RCDCSButton alloc] initWithFrame:CGRectMake(X, CGRectGetMaxY(self.solveTitle.frame) + 11,
                                                                      button_size.width, button_size.height)];
        [_solvedButton addTarget:self
                          action:@selector(didClickSolveButton:)
...
- (RCDCSButton *)noSolveButton {
    if (!_noSolveButton) {
        _noSolveButton = [[RCDCSButton alloc]
            initWithFrame:CGRectMake(CGRectGetMaxX(self.solvedButton.frame) + button_space,
                                     CGRectGetMaxY(self.solveTitle.frame) + 11, button_size.width, button_size.height)];
        [_noSolveButton addTarget:self
                           action:@selector(didClickNoSolveButton:)
                 forControlEvents:(UIControlEventTouchUpInside)];
