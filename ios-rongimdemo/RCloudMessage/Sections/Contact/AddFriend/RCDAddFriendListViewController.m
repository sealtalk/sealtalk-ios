...
@interface RCDAddFriendListViewController ()<UITableViewDelegate, UITableViewDataSource, UIAlertViewDelegate, RCDWeChatManagerDelegate, UISearchBarDelegate, RCDMyQRCodeViewDelegate>
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    switch (indexPath.row) {
        ...
        case 1: {
            ...
            break;
        case 2: {
            [self showAlert:RCDLocalizedString(@"InviteWeChatFriend") message:RCDLocalizedString(@"ToInviteWeChatFriend") cancelBtnTitle:RCDLocalizedString(@"cancel") otherBtnTitle:RCDLocalizedString(@"ConfirmBtnTitle") tag:10001];
        }
            break;
        case 3: {
...
- (void)shareUrlToWeChat {
    ...
    if ([RCDWeChatManager weChatCanShared]) {
        [[RCDWeChatManager sharedManager] sendLinkContent:RCDQRCodeContentInfoUrl title:RCDLocalizedString(@"WXShare_RC_Title") description:RCDLocalizedString(@"WXShare_RC_Detail") thumbImage:logoImage atScene:WXSceneSession];
    } else {
        [self showAlert:nil message:RCDLocalizedString(@"NotInstalledWeChat") cancelBtnTitle:nil otherBtnTitle:RCDLocalizedString(@"ConfirmBtnTitle") tag:0];
    }
}
...
- (UIView *)myQRCodeView {
    if (!_myQRCodeView) {
        ...
        UITapGestureRecognizer *tap = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(tapMyQRCode)];
