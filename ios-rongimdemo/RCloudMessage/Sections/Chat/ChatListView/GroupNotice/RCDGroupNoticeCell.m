...
- (void)setDataModel:(RCDGroupNotice *)model{
    ...
    if (!group) {
        [RCDGroupManager getGroupInfoFromServer:model.groupId complete:^(RCDGroupInfo *groupInfo) {
...
- (void)setGroupInfo:(RCDGroupInfo *)group{
    if (self.notice.noticeType == RCDGroupNoticeTypeInviteeApproving) {
        ...
        [self.portraitImageView sd_setImageWithURL:[NSURL URLWithString:group.portraitUri] placeholderImage:[RCKitUtility imageNamed:@"default_group_portrait" ofBundle:@"RongCloud.bundle"]];
...
- (void)setUserInfo{
    if (self.notice.noticeType == RCDGroupNoticeTypeInviteeApproving) {
        [self getUserInfo:self.notice.operatorId complete:^(RCDUserInfo *user) {
            self.infoLabel.text = [NSString stringWithFormat:RCDLocalizedString(@"InviteYouJoinGroup"),user.name];
        }];
    }else if(self.notice.noticeType == RCDGroupNoticeTypeManagerApproving){
        [self getUserInfo:self.notice.targetId complete:^(RCDUserInfo *user) {
...
- (UIButton *)agreeButton{
    if (!_agreeButton) {
        ...
        [_agreeButton addTarget:self action:@selector(didClickAgreeButton) forControlEvents:(UIControlEventTouchUpInside)];
