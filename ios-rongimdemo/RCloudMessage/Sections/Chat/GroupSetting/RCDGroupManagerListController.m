...
- (UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section{
    ...
    if (section == 0) {
        ...
        label.text = [NSString stringWithFormat:@"   %@(%lu/5)",RCDLocalizedString(@"GroupManagerTitle"),(unsigned long)self.managers.count];
    }
    return label;
}
...
- (void)didSelectUserId:(NSString *)userId{
    ...
    [NormalAlertView showAlertWithMessage:[NSString stringWithFormat:RCDLocalizedString(@"GroupManagerDeleteTitle"),name]  highlightText:name leftTitle:RCDLocalizedString(@"cancel") rightTitle:RCDLocalizedString(@"confirm") cancel:^{
...
- (void)deleteGroupManager:(NSString *)userId{
    ...
    //删除管理员
    [RCDGroupManager removeGroupManagers:@[userId] groupId:self.groupId complete:^(BOOL success) {
