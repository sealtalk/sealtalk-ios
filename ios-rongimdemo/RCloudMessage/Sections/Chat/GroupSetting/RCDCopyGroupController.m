...
- (void)setGroupInfo{
    ...
    if (self.group.portraitUri.length > 0) {
        [self.portraitImageView sd_setImageWithURL:[NSURL URLWithString:self.group.portraitUri] placeholderImage:[RCKitUtility imageNamed:@"default_group_portrait" ofBundle:@"RongCloud.bundle"]];
    }
...
- (void)didClickConfirmAction{
    [NormalAlertView showAlertWithMessage:RCDLocalizedString(@"IsCopyGroup") highlightText:nil leftTitle:RCDLocalizedString(@"cancel") rightTitle:RCDLocalizedString(@"confirm") cancel:^{
...
- (void)copyGroup{
    ...
    [RCDGroupManager copyGroup:self.groupId groupName:groupName portraitUri:nil complete:^(NSString *groupId, RCDGroupAddMemberStatus status) {
...
- (UIButton *)confirmBotton{
    if (!_confirmBotton) {
        ...
        [_confirmBotton addTarget:self action:@selector(didClickConfirmAction) forControlEvents:(UIControlEventTouchUpInside)];
