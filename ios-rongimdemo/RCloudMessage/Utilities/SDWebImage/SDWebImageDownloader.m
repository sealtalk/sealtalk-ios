...
+ (void)initialize {
    ...
    // http://github.com/rs/SDNetworkActivityIndicator )
    // To use it, just add #import "SDNetworkActivityIndicator.h" in addition to
    // the SDWebImage import
...
- (id)init {
    if ((self = [super init])) {
        ...
        _barrierQueue =
            dispatch_queue_create("com.hackemist.SDWebImageDownloaderBarrierQueue", DISPATCH_QUEUE_CONCURRENT);
    ...
    [self addProgressCallback:progressBlock
            andCompletedBlock:completedBlock
               ...
               createCallback:^{
                   ...
                   // In order to prevent from potential duplicate caching
                   // (NSURLCache + SDImageCache) we disable the cache for image
                   // requests if told otherwise
