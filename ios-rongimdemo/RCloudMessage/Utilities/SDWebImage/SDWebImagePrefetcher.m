...
- (void)startPrefetchingAtIndex:(NSUInteger)index {
  ...
  self.requestedCount++;
  [self.manager
      downloadImageWithURL:self.prefetchURLs[index]
                 ...
                 completed:^(UIImage *image, NSError *error,
                   ...
                   if (image) {
                     if (self.progressBlock) {
                       self.progressBlock(self.finishedCount,
                                          [self.prefetchURLs count]);
                     }
                     NSLog(@"Prefetched %@ out of %@", @(self.finishedCount),
                           @(self.prefetchURLs.count));
                   } else {
                     if (self.progressBlock) {
                       self.progressBlock(self.finishedCount,
                     ...
                     NSLog(@"Prefetched %@ out of %@ (Failed)",
                           @(self.finishedCount), @(self.prefetchURLs.count));
                   ...
                   if (self.prefetchURLs.count > self.requestedCount) {
                     ...
                     if (self.completionBlock) {
                       self.completionBlock(self.finishedCount,
                                            self.skippedCount);
...
- (void)reportStatus {
  ...
  NSLog(@"Finished prefetching (%@ successful, %@ skipped, timeElasped %.2f)",
        @(total - self.skippedCount), @(self.skippedCount),
        CFAbsoluteTimeGetCurrent() - self.startedTime);
...
- (void)prefetchURLs:(NSArray *)urls
  ...
  if (urls.count == 0) {
    ...
    for (NSUInteger i = 0;
         i < self.maxConcurrentDownloads && self.requestedCount < listCount;
         i++) {
