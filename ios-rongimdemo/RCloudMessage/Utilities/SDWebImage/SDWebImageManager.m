...
- (void)cachedImageExistsForURL:(NSURL *)url
  ...
  BOOL isInMemoryCache =
      ([self.imageCache imageFromMemoryCacheForKey:key] != nil);
  ...
  [self.imageCache diskImageExistsWithKey:key
                               completion:^(BOOL isInDiskCache) {
                                 // the completion block of
                                 // checkDiskCacheForImageWithKey:completion: is
                                 // always called on the main queue, no need to
                                 // further dispatch
                                 if (completionBlock) {
                              ...
                              ^(UIImage *image, SDImageCacheType cacheType) {
                                ...
                                if ((!image ||
                                    ...
                                    (![self.delegate
                                     ...
                                     [self.delegate imageManager:self
                                         shouldDownloadImageForURL:url])) {
                                  if (image &&
                                      options & SDWebImageRefreshCached) {
                                    dispatch_main_sync_safe(^{
                                      // If image was found in the cache bug
                                      // SDWebImageRefreshCached is provided,
                                      // notify about the cached image
                                      // AND try to re-download it in order to
                                      // let a chance to NSURLCache to refresh
                                      // it from server.
                                  ...
                                  // download if no image or requested to
                                  // refresh anyway, and download allowed by
                                  // delegate
                                  ...
                                  if (image &&
                                      options & SDWebImageRefreshCached) {
                                    // force progressive off if image already
                                    // cached but forced refreshing
                                    downloaderOptions &=
                                        ~SDWebImageDownloaderProgressiveDownload;
                                    // ignore image read from NSURLCache if
                                    // image if cached but force refreshing
                                    downloaderOptions |=
                                                  ...
                                                  .imageDownloader downloadImageWithURL:
                                                                       url
                                                                              ...
                                                                              completed:
                                                                                  ^(UIImage
                                                                                        *downloadedImage,
                                                                                    NSData *data, NSError *error, BOOL
                                                                                                                      finished) {
                                                                                    if (weakOperation
                                                                                            .isCancelled) {
                                  ...
                                  // Image not in cache and download disallowed
                                  // by delegate
                                  dispatch_main_sync_safe(^{
                                    if (!weakOperation.isCancelled) {
                                      completedBlock(nil, nil,
                                                     SDImageCacheTypeNone, YES,
                                                     url);
                                    }
                                  });
                                  @synchronized(self.runningOperations) {
...
- (void)setCancelBlock:(SDWebImageNoParamsBlock)cancelBlock {
  // check if the operation is already cancelled, then we just call the
  // cancelBlock
  if (self.isCancelled) {
    if (cancelBlock) {
      cancelBlock();
    }
    _cancelBlock = nil; // don't forget to nil the cancelBlock, otherwise we
                        // will get crashes
  } else {
...
- (void)cancel {
  ...
  if (self.cancelBlock) {
    ...
    // Until we can figure the exact cause of the crash, going with the ivar
