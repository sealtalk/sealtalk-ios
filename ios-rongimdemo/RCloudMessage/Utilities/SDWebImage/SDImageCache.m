  ...
  if (toDisk) {
    dispatch_async(self.ioQueue, ^{
      ...
      if (image && (recalculate || !data)) {
        ...
        // We assume the image is PNG, in case the imageData is nil (i.e. if
        // trying to save a UIImage directly),
      ...
      if (data) {
        ...
        [_fileManager createFileAtPath:[self defaultCachePathForKey:key]
                              contents:data
                            attributes:nil];
      }
    });
...
- (BOOL)diskImageExistsWithKey:(NSString *)key {
  ...
  // this is an exception to access the filemanager on another queue than
  // ioQueue, but we are using the shared instance
  // from apple docs on NSFileManager: The methods of the shared NSFileManager
  // object can be called from multiple threads safely.
  exists = [[NSFileManager defaultManager]
      fileExistsAtPath:[self defaultCachePathForKey:key]];
...
- (void)diskImageExistsWithKey:(NSString *)key
  ...
  dispatch_async(_ioQueue, ^{
    BOOL exists =
        [_fileManager fileExistsAtPath:[self defaultCachePathForKey:key]];
    if (completionBlock) {
...
- (NSUInteger)getSize {
  ...
  dispatch_sync(self.ioQueue, ^{
    NSDirectoryEnumerator *fileEnumerator =
        [_fileManager enumeratorAtPath:self.diskCachePath];
