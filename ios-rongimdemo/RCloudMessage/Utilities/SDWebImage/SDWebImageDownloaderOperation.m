...
NSString *const SDWebImageDownloadReceiveResponseNotification =
...
NSString *const SDWebImageDownloadFinishNotification =
...
- (id)initWithRequest:(NSURLRequest *)request
  ...
  if ((self = [super init])) {
    ...
    _finished = NO;
    _expectedSize = 0;
    responseFromCached = YES; // Initially wrong until
                              // `connection:willCacheResponse:` is called or
                              // not called
  }
  return self;
}
...
- (void)start {
  ...
  if (self.connection) {
    ...
    if (floor(NSFoundationVersionNumber) <= NSFoundationVersionNumber_iOS_5_1) {
      // Make sure to run the runloop in our background thread so it can process
      // downloaded data
      // Note: we use a timeout to work around an issue with NSURLConnection
      // cancel under iOS 5
      //       not waking up the runloop, leading to dead threads (see
      //       https://github.com/rs/SDWebImage/issues/466)
    ...
    if (self.completedBlock) {
      self.completedBlock(
          nil, nil, [NSError errorWithDomain:NSURLErrorDomain
                                        code:0
                                    userInfo:@{
...
- (void)cancelInternal {
  ...
  if (self.connection) {
    ...
    // As we cancelled the connection, its callback won't be called and thus
    // won't
...
didReceiveResponse:(NSURLResponse *)response {
  ...
  if (![response respondsToSelector:@selector(statusCode)] ||
    ...
    NSInteger expected = response.expectedContentLength > 0
                             ? (NSInteger)response.expectedContentLength
                             : 0;
...
- (void)connection:(NSURLConnection *)connection didReceiveData:(NSData *)data {
  ...
  if ((self.options & SDWebImageDownloaderProgressiveDownload) &&
      self.expectedSize > 0 && self.completedBlock) {
    ...
    CGImageSourceRef imageSource =
        CGImageSourceCreateWithData((__bridge CFDataRef)self.imageData, NULL);
    ...
    if (width + height == 0) {
      ...
      if (properties) {
        ...
        // oriented incorrectly sometimes. (Unlike the image born of
        // initWithData
        // in connectionDidFinishLoading.) So save it here and pass it on later.
        orientation = [[self class]
            orientationFromPropertyValue:(orientationValue == -1
                                              ? 1
                                              : orientationValue)];
      }
    }
    ...
    if (width + height > 0 && totalSize < self.expectedSize) {
      ...
      if (partialImageRef) {
        ...
        CGContextRef bmContext = CGBitmapContextCreate(
            NULL, width, height, 8, width * 4, colorSpace,
            kCGBitmapByteOrderDefault | kCGImageAlphaPremultipliedFirst);
        CGColorSpaceRelease(colorSpace);
        if (bmContext) {
          CGContextDrawImage(bmContext, (CGRect){.origin.x = 0.0f,
                                                 .origin.y = 0.0f,
                                                 .size.width = width,
                                                 .size.height = partialHeight},
                             partialImageRef);
      ...
      if (partialImageRef) {
        ...
        NSString *key =
            [[SDWebImageManager sharedManager] cacheKeyForURL:self.request.URL];
...
- (NSCachedURLResponse *)connection:(NSURLConnection *)connection
  ...
  responseFromCached = NO; // If this method is called, it means the response
                           // wasn't read from cache
