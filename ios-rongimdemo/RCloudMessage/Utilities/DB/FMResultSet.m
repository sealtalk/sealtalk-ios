    ...
    if (outErr) {
      if (_parentDB) {
        *outErr = [_parentDB lastError];
        ...
        // If 'next' or 'nextWithError' is called after the result set is
        // closed,
        // we need to return the appropriate error.
        NSDictionary *errorMessage =
            [NSDictionary dictionaryWithObject:@"parentDB does not exist"
                                        forKey:NSLocalizedDescriptionKey];
        *outErr = [NSError errorWithDomain:@"FMDatabase"
                                      code:SQLITE_MISUSE
...
- (NSData *)dataForColumnIndex:(int)columnIdx {
  ...
  const char *dataBuffer =
      sqlite3_column_blob([_statement statement], columnIdx);
  ...
  return [NSData dataWithBytes:(const void *)dataBuffer
