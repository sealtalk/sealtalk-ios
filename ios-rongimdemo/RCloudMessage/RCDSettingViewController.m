...
- (void)viewDidLoad {
  ...
  if (self.conversationType == ConversationType_DISCUSSION) {
    ...
    [[RCIMClient sharedClient] getDiscussion:self.targetId
        completion:^(RCDiscussion *discussion) {
          if (discussion) {

            if ([[RCIMClient sharedClient]
    ...
    selectPersonVC.clickDoneCompletion = ^(
      ...
      if (selectedUsers && selectedUsers.count) {
        ...
        NSString *_currentTargetId =
            [RCIMClient sharedClient].currentUserInfo.userId;

        [RCDHTTPTOOL
            getUserInfoByUserID:_currentTargetId
                     completion:^(RCUserInfo *user) {
                       RCUserInfo *userInfo = user;
                       NSMutableArray *_allUsers = [NSMutableArray new];
                       [_allUsers
                           addObject:userInfo]; // add current user info object
                       [_allUsers
                           addObjectsFromArray:selectedUsers]; // add the
                                                               // selected users
                       [self addUsers:(NSArray *)_allUsers];

                       [self createDiscussionOrInvokeMemberWithSelectedUsers:
                                 selectedUsers];
      ...
      if (_count > 1) {
        ...
        for (int i = 0; i < _count; i++) {
          ...
          if (i == (_count - 1)) {
            ...
            [discussionTitle
                appendString:[NSString stringWithFormat:@"%@%@", _userInfo.name,
                                                        @","]];
          }
        ...
        [[RCIMClient sharedClient] createDiscussion:discussionTitle
...
- (void)setSwitchState {
  ...
  RCConversation *conversation =
      [[RCIMClient sharedClient] getConversation:self.conversationType
                                        targetId:self.targetId];
...
- (void)onClickClearMessageHistory:(id)sender {
  ...
  UIActionSheet *actionSheet =
      [[UIActionSheet alloc] initWithTitle:@"是否删除历史消息？"
                                  delegate:self
...
- (void)onClickIsTopSwitch:(id)sender {
  UISwitch *swch = sender;
  [[RCIMClient sharedClient] setConversationToTop:self.conversationType
                                         targetId:self.targetId
                                            isTop:swch.on];
}
...
- (void)actionSheet:(UIActionSheet *)actionSheet
  ...
  if (buttonIndex == 0) {
    BOOL isClear =
        [[RCIMClient sharedClient] clearMessages:self.conversationType
                                        targetId:self.targetId];
