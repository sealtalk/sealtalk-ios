...
@interface RCDFriendListViewController ()<UITableViewDelegate,UITableViewDataSource,UISearchBarDelegate,UISearchDisplayDelegate>
{
...
- (void)configTableView {
    ...
    self.tableView=[[UITableView alloc]initWithFrame:CGRectMake(0.0, 0.0, kScreenWidth, kScreenHeight) style:UITableViewStylePlain];
...
- (void)loadFriendsList:(NSArray *)friendsList {
    ...
    dispatch_async(queue, ^{
        ...
        for (NSString *friendId in friendsList) {
            if (!([RCIM sharedRCIM].currentUserInfo && [friendId isEqualToString:[RCIM sharedRCIM].currentUserInfo.userId])) {
...
- (void)configNav{
    ...
    UILabel *backText =
    [[UILabel alloc] initWithFrame:CGRectMake(0, 0, 70, 22)];
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (tableView==_searchDisplayController.searchResultsTableView){
        ...
        if([user.portraitUri hasPrefix:@"file"]){
            ...
            [cell.headImageView sd_setImageWithURL:[NSURL URLWithString:user.portraitUri] placeholderImage:[UIImage imageNamed:@"icon_person"]];
        }
...
- (void)searchBarTextDidBeginEditing:(UISearchBar *)searchBar{
    ...
    for (id view in subViews) {
        if ([view isKindOfClass:[UIButton class]]) {
            ...
            [cancelbutton setTitle:NSLocalizedStringFromTable(@"Cancel", @"RongCloudKit", nil) forState:UIControlStateNormal];
            break;
        }
    }
...
- (void)filterContentForSearchText:(NSString*)searchText scope:(NSString*)scope {
    ...
    for (int i = 0; i < self.dataArr.count; i++) {
        ...
        NSString *storeImageString=[(RCUserInfo *)self.dataArr[i] portraitUri]?[(RCUserInfo *)self.dataArr[i] portraitUri]:@"";
