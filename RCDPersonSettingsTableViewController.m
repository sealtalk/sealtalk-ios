...
- (void)viewDidLoad {
  ...
  TitleList =
      [NSArray arrayWithObjects:@"消息免打扰", @"会话置顶",
                                @"黑名单", @"清除聊天纪录", nil];
  self.tableView.tableFooterView = [UIView new];

  /******************添加headerview*******************/
  UIView *hearView = [[UIView alloc]
  ...
  RCUserInfo *user =
      [[RCDataBaseManager shareInstance] getUserByUserId:_targetId];
  if ([user.portraitUri isEqualToString:@""]) {
    ...
    [portraitImageView sd_setImageWithURL:[NSURL URLWithString:user.portraitUri]
                         placeholderImage:[UIImage imageNamed:@"icon_person"]];
  }
  nameLabel.text = user.name;

  NSDictionary *views =
      NSDictionaryOfVariableBindings(portraitImageView, nameLabel);

  [hearView
      addConstraints:[NSLayoutConstraint constraintsWithVisualFormat:
                                             @"V:|-10-[portraitImageView]-10-|"
                                                             options:0
                                                             metrics:nil
                                                               views:views]];

  [hearView addConstraints:[NSLayoutConstraint
                               constraintsWithVisualFormat:@"V:[nameLabel(20)]"
                                                   options:0
                                                   metrics:nil
                                                     views:views]];

  [hearView addConstraints:
                [NSLayoutConstraint
                    constraintsWithVisualFormat:
                        @"H:|-10-[portraitImageView(65)]-10-[nameLabel]-10-|"
                                        options:0
                                        metrics:nil
                                          views:views]];
...
- (void)clickIsTopBtn:(id)sender {
  [[RCIMClient sharedRCIMClient]
      setConversationToTop:ConversationType_PRIVATE
                  targetId:_targetId
...
- (UITableViewCell *)tableView:(UITableView *)tableView
  ...
  RCDPersonSettingsTableViewCell *cell =
      (RCDPersonSettingsTableViewCell *)[tableView
  ...
  case 2: {
    ...
    [cell.SwitchBtn addTarget:self
                       action:@selector(clickAddOrRemoveToBlacklistBtn:)
             forControlEvents:UIControlEventValueChanged];
  } break;
